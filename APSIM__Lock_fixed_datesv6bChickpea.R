...
site        <- "Lock"
file_path2  <- "/APSIM_runs/Legume/"
file_name   <- "Lock_fixed_legume.db"


con <- dbConnect(SQLite(),  "N:/work/RiskWise/early_sowing/Lock/APSIM_runs/Legume/Lock_fixed_legume.db")
dbListTables(con)
...
germination_date <- Early_sowing_Lock_met_decile %>% 
  filter(Chickpea.Phenology.CurrentStageName == "Germination") %>% 
...
Early_sowing_Lock_met_decile_harvest <- Early_sowing_Lock_met_decile %>% 
   filter(Chickpea.Phenology.CurrentStageName == "HarvestRipe") %>% 
   select(year, StartDate, decile, Yield, Chickpea.Phenology.CurrentStageName, Clock.Today, sowing_date ) %>% 
  rename(harvest_date = Clock.Today
         ) %>% 
  select(-Chickpea.Phenology.CurrentStageName)
...
write_csv(Quantile_sow_date_decile, 
          file =paste0(
            "N:/work/Riskwise/early_sowing/Lock/For_Decile_cal/Legume/",
...
path_saved_files <- file_path_input_data<-file.path("N:","work", "Riskwise", "early_sowing", "Lock", "Results" , "Legume")
...
plot1_box <- ES_Lock_met_decile_harvest_germ %>% 
  ...
  labs(title = "Yield vs early sowing dates Lock 18046.\nMust sow rule. No frost rule used or penalty applied",
       subtitle = "Legume crop, unadjusted climate file",
       y = "Yield",
...
ggsave(plot = plot1_box,
       filename = paste0(path_saved_files,"/Box_yield_vs_Sowing_dates_Lock_Legume", ".png" ),
...
plot1_hisrogram <-  ES_Lock_met_decile_harvest_germ %>% 
  ...
  labs(title = "Lock Histogram Legume Yield - No adjustment",
...
ggsave(plot = plot1_hisrogram,
       filename = paste0(path_saved_files,"/histogram_Frostyield_and_yldvs_Sowing_dates_Lock_Legume", ".png" ),
       width = 20, height = 12, units = "cm")
